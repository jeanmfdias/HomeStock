...
- **Equal-height stretch trap**: when sibling cards/columns sit inside a parent `display: grid` or `display: flex` (default `align-items: stretch`), and each card itself uses `display: grid` without `align-content: start`, taller cards will stretch the shorter cards' grid rows — inflating inputs, buttons, and gaps inconsistently between columns that hold different numbers of children. Fix by switching the inner card to `display: flex; flex-direction: column`, or pinning control sizes (`height` on inputs, explicit width on buttons), or setting `align-content: start` on the inner grid. Always verify control sizing across columns when cards share a row.
- **Button label alignment trap**: a shared `.button` rule that only sets `min-height` and `padding: 0 …` will look fine on `<button>` elements (text centers by default) but misalign on `<a>` / `<RouterLink>` styled with the same class — the label sits at the top of the box because anchors don't center inline content vertically. Always add `display: inline-flex; align-items: center; justify-content: center; line-height: 1` to the shared button base so `<button>`, `<a>`, and `<RouterLink>` render labels identically. Grep `class="(primary|ghost|danger)-button"` and check whether each element is a `<button>` or a link — mixed usage is a tell-tale sign of this bug.
